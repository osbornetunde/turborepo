...
function urlToFile(url, fileExtension, doNotCreateExtraDirectory) {
    const { pathnameOriginal, searchOriginal, hashOriginal } = (0, parseUrl_1.parseUrl)(url, baseServer);
    if (url.startsWith('/')) {
        (0, assert_1.assert)(url === `${pathnameOriginal}${searchOriginal || ''}${hashOriginal || ''}`, { url });
    }
    const hasTrailingSlash = pathnameOriginal.endsWith('/');
    let pathnameModified;
    if (doNotCreateExtraDirectory && pathnameOriginal !== '/') {
        if (hasTrailingSlash) {
            pathnameModified = (0, slice_1.slice)(pathnameOriginal, 0, -1);
        }
        else {
            pathnameModified = pathnameOriginal;
        }
        (0, assert_1.assert)(!pathnameModified.endsWith('/'), { url });
        (0, assert_1.assert)(pathnameModified !== '');
    }
    else {
