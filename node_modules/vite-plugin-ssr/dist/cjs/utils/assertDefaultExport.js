...
function assertDefaultExportObject(fileExports, filePath) {
    ...
    (0, assert_1.assertUsage)((0, isObject_1.isObject)(defaultExport), `${filePath} should default export an object: its \`export default\` is a \`${typeof defaultExport}\` but it should be an object instead`);
}
...
function assertSingleDefaultExport(fileExports, filePath, defaultExportValueIsUnknown) {
    ...
    if (exportsInvalid.length === 0) {
        if (exportsHasDefault) {
            ...
            (0, assert_1.assert)(exportsRelevant.length === 0);
            (0, assert_1.assertUsage)(false, `${filePath} doesn't export any value, but it should have a \`export default\` instead`);
        }
    }
        ...
        if (defaultExportValueIsUnknown) {
            (0, assert_1.assertWarning)(exportsInvalid.length === 0, `${filePath} should only have a default export: remove \`export { ${exportsInvalidStr} }\``, { onlyOnce: true, showStackTrace: false });
        }
        else {
            (0, assert_1.assertWarning)(exportsInvalid.length === 0, `${filePath} replace \`export { ${exportsInvalidStr} }\` with \`export default { ${exportsInvalidStr} }\``, { onlyOnce: true, showStackTrace: false });
        }
    }
}
