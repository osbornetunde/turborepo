...
const assertPageFilePath_1 = require("../assertPageFilePath");
...
function determine(pageFilesAll, pageId, envIsClient) {
    ...
    const getPageIdFile = (iso) => {
        ...
        (0, utils_1.assertUsage)(files.length <= 1, `Merge the following files into a single file: ${files.map((p) => p.filePath).join(' ')}`);
        const pageIdFile = files[0];
        (0, utils_1.assert)(pageIdFile === undefined || !pageIdFile.isDefaultPageFile);
        return pageIdFile;
    };
    ...
    const pageFiles = [pageIdFileEnv, pageIdFileIso, ...defaultFilesNonRenderer, rendererFileEnv, rendererFileIso].filter(utils_1.isNotNullish);
    return pageFiles;
}
...
function getPathDistance(pathA, pathB) {
    (0, assertPageFilePath_1.assertPageFilePath)(pathA);
    (0, assertPageFilePath_1.assertPageFilePath)(pathB);
