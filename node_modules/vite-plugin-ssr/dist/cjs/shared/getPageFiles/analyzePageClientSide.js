...
const analyzeExports_1 = require("./analyzePageClientSide/analyzeExports");
const determineClientEntry_1 = require("./analyzePageClientSide/determineClientEntry");
const getAllPageIdFiles_1 = require("./getAllPageIdFiles");
const getAllPageIdFiles_2 = require("./getAllPageIdFiles");
const utils_1 = require("../utils");
const getExportNames_1 = require("./analyzePageClientSide/getExportNames");
...
function analyzePageClientSide(pageFilesAll, pageId) {
    let pageFilesClientSide = (0, getAllPageIdFiles_1.getPageFilesClientSide)(pageFilesAll, pageId);
    const pageFilesServerSide = (0, getAllPageIdFiles_2.getPageFilesServerSide)(pageFilesAll, pageId);
    const { isHtmlOnly, isClientRouting } = (0, analyzeExports_1.analyzeExports)({ pageFilesClientSide, pageFilesServerSide, pageId });
    if (isHtmlOnly) {
        ...
        pageFilesClientSide = pageFilesClientSide.filter((p) => p.isEnv('CLIENT_ONLY') && !(0, getExportNames_1.getExportNames)(p).includes('render'));
        pageFilesClientSide = removeOverridenPageFiles(pageFilesClientSide);
    }
    const { clientEntries, clientDependencies } = (0, determineClientEntry_1.determineClientEntry)({
        pageFilesClientSide,
        pageFilesServerSide,
        isHtmlOnly,
        isClientRouting
...
async function analyzePageClientSideInit(pageFilesAll, pageId, { sharedPageFilesAlreadyLoaded }) {
    const pageFilesClientSide = (0, getAllPageIdFiles_1.getPageFilesClientSide)(pageFilesAll, pageId);
    await Promise.all(pageFilesClientSide.map(async (p) => {
        (0, utils_1.assert)(p.isEnv('CLIENT_ONLY') || p.isEnv('CLIENT_AND_SERVER'));
...
function removeOverridenPageFiles(pageFilesClientSide) {
    ...
    for (const p of pageFilesClientSide) {
        ...
        if ((0, getExportNames_1.getExportNames)(p).includes('overrideDefaultPages')) {
            break;
        }
    }
