...
function analyzeExports({ pageFilesClientSide, pageFilesServerSide, pageId }) {
    ...
    function isHtmlOnly() {
        ...
        {
            const definesClientRenderer = pageFilesClientSide.some((p) => p.pageId === pageId && p.fileType === '.page.client' && (0, getExportNames_1.getExportNames)(p).includes('render'));
    ...
    function assertClientSideRenderHook() {
        const hasClientSideRenderHook = pageFilesClientSide.some((p) => {
            return (0, getExportNames_1.getExportNames)(p).includes('render');
        });
        (0, utils_1.assertUsage)(hasClientSideRenderHook, [
    ...
    function isClientRouting() {
        const hasClientRoutingExport = pageFilesClientSide.some((p) => {
            return (0, getExportNames_1.getExportNames)(p).includes('clientRouting');
        });
