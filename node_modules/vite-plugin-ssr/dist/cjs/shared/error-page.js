...
function getErrorPageId(pageFilesAll, pageConfigs) {
    if (pageConfigs.length > 0) {
        ...
        if (errorPageConfigs.length === 0)
            return null;
        (0, utils_1.assertUsage)(errorPageConfigs.length === 1, 'Only one error page can be defined');
        return errorPageConfigs[0].pageId;
    }
    // TODO/v1-release: remove
    const errorPageIds = (0, utils_1.unique)(pageFilesAll.map(({ pageId }) => pageId).filter((pageId) => isErrorPageId(pageId, false)));
    (0, utils_1.assertUsage)(errorPageIds.length <= 1, `Only one _error.page.js is allowed, but found several: ${errorPageIds.join(' ')}`);
