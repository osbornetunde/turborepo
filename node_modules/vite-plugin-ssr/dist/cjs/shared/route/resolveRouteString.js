...
exports.analyzeRouteString = exports.isStaticRouteString = exports.getUrlFromRouteString = exports.resolveRouteString = void 0;
...
function assertRouteString(routeString, errMsgPrefix) {
    (0, utils_2.assertUsage)(routeString.startsWith('/') || routeString === '*', (() => {
        if (!errMsgPrefix) {
...
function resolveRouteString(routeString, urlPathname) {
    ...
    (0, utils_2.assert)(urlPathname.startsWith('/'));
    ...
    for (let i = 0; i < Math.max(routeSegments.length, urlSegments.length); i++) {
        ...
        if (routeSegment === '*') {
            ...
            (0, utils_1.assertWarning)(!routeSegment.startsWith(PARAM_TOKEN_OLD), `Outdated Route String \`${routeString}\`, use \`${routeString
...
function getUrlFromRouteString(routeString) {
    (0, utils_2.assert)(routeString.startsWith('/'));
...
function assertGlob(routeString) {
    ...
    (0, utils_2.assertUsage)(numberOfGlobChars <= 1, `Invalid Route String \`${routeString}\`: Route Strings are not allowed to contain more than one glob character \`*\`.`);
    (0, utils_2.assertUsage)(numberOfGlobChars === 0 || (numberOfGlobChars === 1 && routeString.endsWith('*')), `Invalid Route String \`${routeString}\`: make sure your Route String ends with the glob character \`*\`.`);
}
