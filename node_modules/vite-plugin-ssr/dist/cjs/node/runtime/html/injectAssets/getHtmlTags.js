...
const sanitizeJson_1 = require("./sanitizeJson");
const inferHtmlTags_1 = require("./inferHtmlTags");
const getViteDevScripts_1 = require("./getViteDevScripts");
const mergeScriptTags_1 = require("./mergeScriptTags");
...
async function getHtmlTags(pageContext, injectToStream, injectFilter) {
    ...
    if (injectFilter && isProduction) {
        ...
        Object.values(injectFilterEntries).forEach((entry) => (0, utils_1.freezePartial)(entry, { inject: (val) => val === false || val === 'HTML_BEGIN' || val === 'HTML_END' }));
        const res = injectFilter(injectFilterEntries);
        (0, utils_1.assertUsage)(res === undefined, 'Wrong injectFilter() usage, see https://vite-plugin-ssr.com/injectFilter');
        ...
        injectFilterEntries.forEach((a) => {
            ...
            */
            if (a.assetType === 'style' && a.isEntry) {
                ...
                (0, utils_1.assertWarning)(a.inject, `[injectFilter()] We recommend against not injecting ${a.src}`, {
                    onlyOnce: true,
                    showStackTrace: false
                });
            }
            if (!isHtmlOnly && a.assetType === 'script') {
                (0, utils_1.assertWarning)(a.inject, `[injectFilter()] We recommend against not preloading JavaScript (${a.src})`, {
                    onlyOnce: true,
                    showStackTrace: false
                });
            }
    ...
    for (const asset of injectFilterEntries) {
        if (asset.assetType !== 'script' && asset.inject) {
            const htmlTag = asset.isEntry ? (0, inferHtmlTags_1.inferAssetTag)(asset) : (0, inferHtmlTags_1.inferPreloadTag)(asset);
...
async function getMergedScriptTag(pageAssets, isProduction) {
    ...
    const viteScripts = await (0, getViteDevScripts_1.getViteDevScripts)();
    const scriptTagsHtml = `${viteScripts}${scriptAssets.map((asset) => (0, inferHtmlTags_1.inferAssetTag)(asset)).join('')}`;
    const scriptTag = (0, mergeScriptTags_1.mergeScriptTags)(scriptTagsHtml, isProduction);
    return scriptTag;
}
...
function assertInjectFilterUsage(injectFilterEntries, stamp) {
    injectFilterEntries.forEach((entry, i) => {
        (0, utils_1.assertUsage)((0, utils_1.isObject)(entry), `[injectFilter()] Entry ${i} isn't an object`);
        (0, utils_1.assertUsage)(typeof entry.src === 'string', `[injectFilter()] Entry ${i} is missing property \`src\``);
        (0, utils_1.assertUsage)(entry[stamp] === true, `[injectFilter()] Entry ${i} (${entry.src}) isn't the original object, see https://vite-plugin-ssr.com/injectFilter`);
        (0, utils_1.assert)([false, 'HTML_BEGIN', 'HTML_END'].includes(entry.inject));
        (0, utils_1.assert)(entry.assetType === null || typeof entry.assetType === 'string');
        (0, utils_1.assert)(entry.mediaType === null || typeof entry.mediaType === 'string');
        (0, utils_1.assert)(typeof entry.isEntry === 'boolean');
        (0, utils_1.assert)(Object.keys(entry).length === 5);
    });
}
