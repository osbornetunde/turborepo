...
exports.pipeNodeStream = exports.pipeWebStream = exports.pipeStream = exports.stampPipe = exports.isStreamWritableNode = exports.isStreamWritableWeb = exports.streamPipeNodeToString = exports.streamReadableWebToString = exports.inferStreamName = exports.getStreamName = exports.isStream = exports.pipeToStreamWritableWeb = exports.pipeToStreamWritableNode = exports.getStreamReadableWeb = exports.getStreamReadableNode = exports.streamToString = exports.processStream = void 0;
...
const debug = (0, utils_1.createDebugger)('vps:stream');
...
function isStreamWritableNode(thing) {
    ...
    // https://stackoverflow.com/questions/17009975/how-to-test-if-an-object-is-a-stream-in-nodejs/37022523#37022523
    return (0, utils_1.hasProp)(thing, 'write', 'function');
}
...
async function processStream(streamOriginal, { injectStringAtBegin, injectStringAtEnd, onErrorWhileStreaming, enableEagerStreaming }) {
    ...
    const { streamWrapper, streamWrapperOperations } = await createStreamWrapper({
        ...
        onData(chunk) {
            (0, utils_1.assert)(streamEnded === false);
    ...
    function resolvePromise() {
        (0, utils_1.assert)(!delayStreamStart()); // The stream promise shouldn't resolve before delayStreamStart()
        (0, utils_1.assert)(wrapperCreated); // Doesn't make sense to resolve streamWrapper if it isn't defined yet
...
async function createStreamWrapper({ streamOriginal, onError, onData, onEnd, onFlush, onReadyToWrite }) {
    if ((0, react_streaming_1.isStreamReactStreaming)(streamOriginal)) {
        debug('onRenderHtml() hook returned `react-streaming` result');
        const stream = (0, react_streaming_1.getStreamFromReactStreaming)(streamOriginal);
    ...
    if (isStreamPipeNode(streamOriginal)) {
        ...
        const writeChunk = (chunk) => {
            (0, utils_1.assert)(writableOriginal);
        ...
        (0, utils_1.objectAssign)(writableProxy, {
            flush: () => {
                onFlush();
            }
        });
        (0, utils_1.assert)(typeof writableProxy.flush === 'function');
    ...
    if (isStreamPipeWeb(streamOriginal)) {
        ...
        const writeChunk = (chunk) => {
            (0, utils_1.assert)(writerOriginal);
...
function isStream(something) {
    if (isStreamReadableWeb(something) ||
        ...
        (0, react_streaming_1.isStreamReactStreaming)(something)) {
...
function pipeWebStream(pipe) {
    (0, utils_1.assertWarning)(false, 'pipeWebStream() is outdated, use stampPipe() instead. See https://vite-plugin-ssr.com/stream', {
        onlyOnce: true,
        showStackTrace: true
    });
...
function getStreamPipeWeb(thing) {
    ...
    if ((0, utils_1.isCallable)(thing) && 'isWebStreamPipe' in thing) {
        // `stampPipe()`
        return thing;
    }
    else {
...
function isStreamPipeWeb(thing) {
    // `pipeWebStream()`
    if ((0, utils_1.isObject)(thing) && __streamPipeWeb in thing) {
        return true;
    }
    // `stampPipe()`
    if ((0, utils_1.isCallable)(thing) && 'isWebStreamPipe' in thing) {
...
function pipeNodeStream(pipe) {
    (0, utils_1.assertWarning)(false, 'pipeNodeStream() is outdated, use stampPipe() instead. See https://vite-plugin-ssr.com/stream', { onlyOnce: true, showStackTrace: true });
...
function getStreamPipeNode(thing) {
    ...
    if ((0, utils_1.isCallable)(thing) && 'isNodeStreamPipe' in thing) {
        // `stampPipe()`
        return thing;
    }
    else {
...
function isStreamPipeNode(thing) {
    // `pipeNodeStream()`
    if ((0, utils_1.isObject)(thing) && __streamPipeNode in thing) {
        return true;
    }
    // `stampPipe()`
    if ((0, utils_1.isCallable)(thing) && 'isNodeStreamPipe' in thing) {
...
function stampPipe(pipe, pipeType) {
    (0, utils_1.assertUsage)(pipeType, 'stampPipe(pipe, pipeType): argument `pipeType` is missing.)');
    (0, utils_1.assertUsage)(['web-stream', 'node-stream'].includes(pipeType), "stampPipe(pipe, pipeType): argument `pipeType` should be either 'web-stream' or 'node-stream'.");
...
async function streamToString(stream) {
    ...
    if ((0, react_streaming_1.isStreamReactStreaming)(stream)) {
        return await (0, react_streaming_1.streamReactStreamingToString)(stream);
    }
    (0, utils_1.assert)(false);
}
...
function getStreamName(type, standard) {
    let standardName = (0, utils_1.capitalizeFirstLetter)(standard);
    if (standardName === 'Node') {
        standardName = 'Node.js';
    }
    const typeName = (0, utils_1.capitalizeFirstLetter)(type);
    if (type !== 'pipe') {
