...
async function renderDocumentHtml(documentHtml, pageContext, onErrorWhileStreaming, injectFilter) {
    if (isEscapedString(documentHtml)) {
        (0, utils_1.objectAssign)(pageContext, { _isStream: false });
        let htmlString = getEscapedString(documentHtml);
        htmlString = await (0, injectAssets_1.injectHtmlTagsToString)([htmlString], pageContext, injectFilter);
        return htmlString;
    }
    if ((0, stream_1.isStream)(documentHtml)) {
        (0, utils_1.objectAssign)(pageContext, { _isStream: true });
    ...
    if (isTemplateWrapped(documentHtml)) {
        ...
        if (!('htmlStream' in render)) {
            ...
            const htmlString = await (0, injectAssets_1.injectHtmlTagsToString)(htmlPartsAll, pageContext, injectFilter);
            return htmlString;
        }
        else {
            (0, utils_1.objectAssign)(pageContext, { _isStream: true });
...
async function renderHtmlStream(streamOriginal, injectString, pageContext, onErrorWhileStreaming, injectFilter) {
    ...
    if (injectString) {
        ...
        const { injectAtStreamBegin, injectAtStreamEnd } = (0, injectAssets_1.injectHtmlTagsToStream)(pageContext, injectToStream, injectFilter);
...
function isTemplateWrapped(something) {
    return (0, utils_1.hasProp)(something, '_template');
}
function isEscapedString(something) {
    const result = (0, utils_1.hasProp)(something, '_escaped');
    if (result) {
        (0, utils_1.assert)((0, utils_1.hasProp)(something, '_escaped', 'string'));
        (0, utils_1.checkType)(something);
    }
    return result;
}
function getEscapedString(escapedString) {
    let htmlString;
    (0, utils_1.assert)((0, utils_1.hasProp)(escapedString, '_escaped'));
...
function _dangerouslySkipEscape(arg) {
    if ((0, utils_1.hasProp)(arg, '_escaped')) {
        (0, utils_1.assert)(isEscapedString(arg));
        return arg;
    }
    (0, utils_1.assertUsage)(!(0, utils_1.isPromise)(arg), `[dangerouslySkipEscape(str)] Argument \`str\` is a promise. It should be a string instead (or a stream). Make sure to \`await str\`.`);
    if (typeof arg === 'string') {
        return { _escaped: arg };
    }
    (0, utils_1.assertWarning)(false, `[dangerouslySkipEscape(str)] Argument \`str\` should be a string but we got \`typeof str === "${typeof arg}"\`.`, {
        onlyOnce: false,
...
async function renderTemplate(templateContent, pageContext) {
    ...
    const setStream = (stream) => {
        ...
        (0, utils_1.assertUsage)(!htmlStream, `Injecting two streams in \`escapeInject\` template tag of ${hookName}() hook defined by ${hookFilePath}. Inject only one stream instead.`);
        htmlStream = stream;
    };
    ...
    for (let i = 0; i < templateVariables.length; i++) {
        ...
        if (templateVar === undefined || templateVar === null) {
            (0, utils_1.assertWarning)(false, getErrMsg(`\`${templateVar}\` which will be converted to an empty string`, `Pass an empty string instead of \`${templateVar}\` to remove this warning.`), { showStackTrace: false, onlyOnce: false });
            templateVar = '';
        }
        {
        ...
        {
            const { isProduction } = (0, globalContext_1.getGlobalContext)();
            if ((0, utils_1.isHtml)(templateVar) && !isProduction) {
                (0, utils_1.assertWarning)(false, getErrMsg(`\`${templateVar}\` which seems to be HTML code`, "Did you forget to wrap the value with dangerouslySkipEscape()? (This warning isn't shown in production.)"), { showStackTrace: false, onlyOnce: false });
            }
        }
    ...
    if (htmlStream === null) {
        (0, utils_1.assert)(htmlPartsEnd.length === 0);
        return {
