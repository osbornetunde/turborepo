...
const injectHtmlTags_1 = require("./injectAssets/injectHtmlTags");
const getHtmlTags_1 = require("./injectAssets/getHtmlTags");
async function injectHtmlTagsToString(htmlParts, pageContext, injectFilter) {
    const htmlTags = await (0, getHtmlTags_1.getHtmlTags)(pageContext, null, injectFilter);
...
function injectHtmlTagsToStream(pageContext, injectToStream, injectFilter) {
    ...
    async function injectAtStreamBegin(htmlPartsBegin) {
        htmlTags = await (0, getHtmlTags_1.getHtmlTags)(pageContext, injectToStream, injectFilter);
        const pageAssets = await pageContext.__getPageAssets();
...
function injectToHtmlBegin(htmlBegin, htmlTags, injectToStream) {
    ...
    htmlBegin = (0, injectHtmlTags_1.createHtmlHeadIfMissing)(htmlBegin);
    htmlBegin = (0, injectHtmlTags_1.injectHtmlTags)(htmlBegin, htmlTagsAtBegin, injectToStream);
    return htmlBegin;
}
function injectToHtmlEnd(htmlEnd, htmlTags) {
    const htmlTagsAtEnd = htmlTags.filter((snippet) => snippet.position === 'HTML_END');
    htmlEnd = (0, injectHtmlTags_1.injectHtmlTags)(htmlEnd, htmlTagsAtEnd, null);
    return htmlEnd;
}
async function resolvePageContextPromise(pageContext) {
    ...
    if ((0, utils_1.isCallable)(pageContextPromise)) {
        pageContextProvidedByUser = await pageContextPromise();
    }
    else if ((0, utils_1.isPromise)(pageContextPromise)) {
        pageContextProvidedByUser = await pageContextPromise;
    }
    else {
        (0, utils_1.assert)(false);
    }
    (0, assertPageContextProvidedByUser_1.assertPageContextProvidedByUser)(pageContextProvidedByUser, {
