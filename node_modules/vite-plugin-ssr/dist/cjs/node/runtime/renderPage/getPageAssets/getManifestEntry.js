...
function getManifestEntry(id, clientManifest, manifestKeyMap) {
    ...
    if (id.startsWith('@@vite-plugin-ssr/')) {
        ...
        (0, utils_1.assert)(manifestEntry && manifestKey, debugInfo);
    ...
    if ((0, virtualFileImportPageCode_1.isVirtualFileIdImportPageCode)(id)) {
        ...
        (0, utils_1.assert)(manifestEntry, debugInfo);
        return { manifestEntry, manifestKey };
    }
    // User files
    if (id.startsWith('/')) {
    ...
    if ((0, utils_1.isNpmPackageImport)(id)) {
        const manifestKey = manifestKeyMap[id];
        const debugInfo2 = { ...debugInfo, manifestKey };
        (0, utils_1.assert)(manifestKey, debugInfo2);
        const manifestEntry = clientManifest[manifestKey];
        (0, utils_1.assert)(manifestEntry, debugInfo2);
        return { manifestEntry, manifestKey };
    }
    ...
    if (id.startsWith('/node_modules/') || id.startsWith('/../')) {
        ...
        (0, utils_1.assert)(manifestKeyEnd, debugInfo);
        (0, utils_1.assert)(!manifestKeyEnd.startsWith('/'), debugInfo);
        manifestKeyEnd = '/' + manifestKeyEnd;
        {
            const { manifestEntry, manifestKey } = findEntryWithKeyEnd(manifestKeyEnd, clientManifest, id);
            if (manifestEntry) {
                (0, utils_1.assert)(manifestKey, debugInfo);
                return { manifestEntry, manifestKey };
            }
        }
        {
            ...
            (0, utils_1.assert)(dirS[0] === '', debugInfo);
...
function findEntryWithKeyEnd(manifestKeyEnd, clientManifest, id) {
    ...
    (0, utils_1.assert)(manifestKeysRelative.length <= 1, debugInfo);
...
function getEntryWithKeyEnd(manifestKeyEnd, clientManifest, id) {
    ...
    (0, utils_1.assert)(manifestKeys.length <= 1, debugInfo);
