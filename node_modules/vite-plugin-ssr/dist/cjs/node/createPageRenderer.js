...
function showWarnings(options) {
    (0, utils_1.assertWarning)(false, '`createPageRenderer()` is outdated, use `renderPage()` instead. See https://vite-plugin-ssr.com/renderPage', { onlyOnce: true, showStackTrace: true });
    const { viteDevServer, root, outDir, isProduction, base, baseAssets } = options;
    {
        ...
        (0, utils_1.assertWarning)(opts.length === 0, `The options ${str(opts)} you passed to \`createPageRenderer()\` have no effect anymore: they are now automatically determined.`, { showStackTrace: false, onlyOnce: true });
    }
    {
        const opts = [base !== undefined && 'base', baseAssets !== undefined && 'baseAssets'].filter(notFalse);
        (0, utils_1.assertWarning)(opts.length === 0, `The options ${str(opts)} you passed to \`createPageRenderer()\` have no effect. See https://vite-plugin-ssr.com/migration-0.4`, { showStackTrace: false, onlyOnce: true });
    }
}
function str(opts) {
    return opts
