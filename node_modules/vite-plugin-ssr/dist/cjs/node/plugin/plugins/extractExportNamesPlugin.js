...
const globalObject = (0, utils_1.getGlobalObject)('extractExportNamesPlugin.ts', {});
function extractExportNamesPlugin() {
    ...
    return {
        ...
        async transform(src, id, options) {
            const isClientSide = !(0, utils_1.viteIsSSR_options)(options);
...
async function getExtractExportNamesCode(src, isClientSide, isProduction, id) {
    const { exportNames, wildcardReExports } = await (0, parseEsModule_1.getExportNames)(src);
...
function addQuery(moduleName, id) {
    if (moduleName.includes('?')) {
        (0, utils_1.assert)(moduleName.split('?').length === 2);
        moduleName = moduleName.replace('?', '?extractExportNames&');
    }
    else {
        const fileExtension = (0, utils_1.getFileExtension)(moduleName);
        if (!fileExtension) {
            (0, utils_1.assert)(extractExportNamesRE.test(id));
            const idReal = id.split('?')[0];
            (0, utils_1.assertUsage)(false, `Modify the re-export of ${idReal}, see https://github.com/brillout/vite-plugin-ssr/issues/864#issuecomment-1537202290`);
        }
