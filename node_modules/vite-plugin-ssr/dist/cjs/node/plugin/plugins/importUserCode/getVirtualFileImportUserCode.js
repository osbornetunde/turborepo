...
const getVirtualFileImportPageConfigs_1 = require("./v1-design/getVirtualFileImportPageConfigs");
...
async function getVirtualFileImportUserCode(id, options, configVps, config, isDev) {
    const idParsed = (0, virtualFileImportUserCode_1.isVirtualFileIdImportUserCode)(id);
    (0, utils_1.assert)(idParsed);
...
async function getCode(config, configVps, isForClientSide, isClientRouting, isPrerendering, isDev, id) {
    ...
    (0, utils_1.assert)(isDev === !isBuild);
    let content = '';
    {
        const globRoots = getGlobRoots(config, configVps);
        (0, utils_1.debugGlob)('Glob roots: ', globRoots);
    ...
    (0, utils_1.debugGlob)(`Glob imports for ${isForClientSide ? 'client' : 'server'}:\n`, content);
    return content;
}
function generateExtensionImports(extensionsImportPaths, isForClientSide, isBuild, isClientRouting, isPrerendering) {
    ...
    extensionsImportPaths
        ...
        .forEach((importPath) => {
        const fileType = (0, fileTypes_1.determineFileType)(importPath);
...
function addImport(importPath, fileType, exportNames, isBuild) {
    ...
    const value = (() => {
        if (!pageFilesVar.endsWith('Eager')) {
            ...
            const { importVar, importStatement } = (0, generateEagerImport_1.generateEagerImport)(`${importPath}${query}`);
...
${await (0, getVirtualFileImportPageConfigs_1.getVirtualFileImportPageConfigs)(config.root, isForClientSide, isDev, id, configVps)}

`;
    fileTypes_1.fileTypes
        .filter((fileType) => fileType !== '.css')
        .forEach((fileType) => {
        (0, utils_1.assert)(fileType !== '.css');
        const { includeImport, includeExportNames } = determineInjection({
            fileType,
            isForClientSide,
            isClientRouting,
...
function getGlobPath(globRoot, fileType) {
    (0, utils_1.assertPosixPath)(globRoot);
    let globPath = [...globRoot.split('/'), '**', `*${fileType}.${utils_1.scriptFileExtensions}`].filter(Boolean).join('/');
