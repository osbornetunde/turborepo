...
function serveExtensionsAssets(middlewares, extensionsAssetsDirs, config) {
    (0, utils_1.assert)(ASSET_DIR === getAsssetsDirConfig(config));
...
function getExtensionsAssetsDir(config, configVps) {
    ...
    if (0 === extensionsWithAssetsDir.length)
        return [];
    (0, utils_1.assertUsage)(ASSET_DIR === getAsssetsDirConfig(config), 'Cannot modify vite.config.js#build.assetsDir while using ' + extensionsWithAssetsDir[0].npmPackageName);
    const extensionsAssetsDir = extensionsWithAssetsDir.map(({ assetsDir }) => {
        (0, utils_1.assert)(assetsDir);
        (0, utils_1.assertPosixPath)(assetsDir);
        return assetsDir;
    });
...
function copyExtensionsAssetsDir(config, extensionsAssetsDirs) {
    (0, utils_1.assert)(ASSET_DIR === getAsssetsDirConfig(config));
    const outDirClient = path_1.default.posix.join(config.root, config.build.outDir);
    (0, utils_1.assert)(outDirClient.endsWith('/client'), outDirClient);
...
function copyAssetFiles(srcDir, destDir) {
    ...
    for (const file of fs_1.default.readdirSync(srcDir)) {
        ...
        else if ((0, isAsset_1.isAsset)(srcFile)) {
            (0, utils_1.assert)(!(0, utils_1.isScriptFile)(srcFile));
            if (!destDirCreated) {
