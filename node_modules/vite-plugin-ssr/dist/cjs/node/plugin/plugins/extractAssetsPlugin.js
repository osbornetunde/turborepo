...
const isAsset_1 = require("../shared/isAsset");
const parseEsModule_1 = require("../shared/parseEsModule");
const removeSourceMap_1 = require("../shared/removeSourceMap");
...
const debug = (0, utils_1.createDebugger)(debugNamespace);
const debugEnabled = (0, utils_1.isDebugEnabled)(debugNamespace);
    ...
    return [
        // This plugin removes all JavaScript from server-side only code, so that only CSS imports remains. (And also satic files imports e.g. `import logoURL from './logo.svg'`).
        {
            ...
            async transform(src, id, options) {
                ...
                (0, utils_1.assert)(configVps.includeAssetsImportedByServer);
                (0, utils_1.assert)(!(0, utils_1.viteIsSSR_options)(options));
                const importStatements = await (0, parseEsModule_1.getImportStatements)(src);
                const moduleNames = getImportedModules(importStatements);
                const code = moduleNames.map((moduleName) => `import '${moduleName}';`).join('\n');
                debugTransformResult(id, code, importStatements);
                return (0, removeSourceMap_1.removeSourceMap)(code);
            }
        },
        // This plugin appends `?extractAssets` to module IDs
        {
            ...
            async resolveId(source, importer, options) {
                if ((0, utils_1.viteIsSSR_options)(options)) {
                    // When building for the server, there should never be a `?extractAssets` query
                    (0, utils_1.assert)(!extractAssetsRE.test(source));
                    (0, utils_1.assert)(importer === undefined || !extractAssetsRE.test(importer));
                    return;
                }
                ...
                (0, utils_1.assertPosixPath)(config.root);
...
function emptyModule(file, importer) {
    ...
    return (0, utils_1.resolveVirtualFileId)(EMPTY_MODULE_ID);
}
function appendExtractAssetsQuery(file, importer) {
    debugOperation('TRANSFORMED', file, importer);
    return (0, extractAssetsQuery_1.extractAssetsAddQuery)(file);
}
...
function debugTransformResult(id, code, importStatements) {
    debug(`source TRANSFORMED: ${id} (CODE: \`${code.split('\n').join(' ')}\`, IMPORTS: ${stringifyImportStatements(importStatements)
        .map((s) => `\`${s}\``)
        .join(', ')})`);
}
