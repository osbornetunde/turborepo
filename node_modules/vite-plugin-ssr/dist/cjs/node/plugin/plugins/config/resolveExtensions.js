...
function resolveExtensions(configs, config) {
    ...
    return extensions.map((extension) => {
        ...
        (0, utils_1.assertUsage)((0, utils_1.isNpmPackageName)(npmPackageName), `vite-plugin-ssr extension '${npmPackageName}' doesn't seem to be a valid npm package name`);
        ...
        let pageConfigsSrcDirResolved = null;
        {
            ...
            if (pageConfigsSrcDir) {
                ...
                (0, utils_1.assert)(pageConfigsSrcDir.endsWith('*'));
                pageConfigsSrcDirResolved = path_1.default.posix.join(npmPackageRootDir, pageConfigsSrcDir.slice(0, -1));
            }
        }
        (0, utils_1.assertUsage)(pageConfigsSrcDirResolved || pageConfigsDistFiles, `Extension ${npmPackageName} should define either extension[number].pageConfigsDistFiles or extension[number].pageConfigsSrcDir`);
        (0, utils_1.assertUsage)(!pageConfigsDistFiles || !pageConfigsSrcDirResolved, `Extension ${npmPackageName} shouldn't define extension[number].pageConfigsDistFiles as well extension[number].pageConfigsSrcDir, it should define only one instead`);
        const assetsDir = (() => {
        ...
        (0, utils_1.assertUsage)(!(assetsDir && pageConfigsSrcDirResolved), `Extension ${npmPackageName} shouldn't define both extension[number].pageConfigsSrcDir and extension[number].assetsDir`);
